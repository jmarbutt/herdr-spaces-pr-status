...
// split and zoomed panes are positioned relative to an existing pane, so herdr
// rejects them without a target. tab, popup and overlay do not take one.
export const PLACEMENTS_NEEDING_TARGET = new Set(['split', 'zoomed']);

export function openPluginPane(entrypoint, { placement, direction, targetPaneId, focus = true, ...opts } = {}) {
  ...
  if (targetPaneId && PLACEMENTS_NEEDING_TARGET.has(placement)) {
    args.push('--target-pane', targetPaneId);
  }
