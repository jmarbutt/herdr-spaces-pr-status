...
export function openPluginPane(entrypoint, opts = {}) {
  // Deliberately no --placement: the CLI's accepted values lag the manifest's,
  // and omitting it lets the manifest's own placement apply.
  return cliJson(['plugin', 'pane', 'open', '--plugin', SOURCE, '--entrypoint', entrypoint, '--focus'], opts);
}
