import { stateGlyph, stateLabel, threadsToken } from './tokens.js';
...
export function renderChecks({
  ...
  const marks = MARKS[style === 'emoji' ? 'emoji' : 'compact'];
  const lines = [];
  ...
  const head = `${stateGlyph(pr.rolled, style)} #${pr.number}${label ? ` ${label}` : ''}  +${pr.additions} -${pr.deletions}`;
  lines.push(` ${head}`);
  ...
  const threads = threadsToken(pr, { explicit: true });
  if (threads) lines.push(`${DIM} ${padTo(threads, inner - 1)}${RESET}`);
  lines.push('');
