import { stateGlyph, threadsToken } from './tokens.js';
...
function itemLine(row, width, style) {
  ...
  const detail = [threadsToken(pr, { explicit: true }), review].filter(Boolean).join(' · ');
  const right = diff ? ` ${diff}` : '';
  const left = `   ${glyph} ${padTo(space.label, 14)} ${padTo(number, 8)} ${detail}`;
