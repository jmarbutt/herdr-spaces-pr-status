...
export function summarize(result) {
  ...
  const { spaces, repoQueries, branchQueries, cacheHits, reported, failed = 0 } = result.stats;
  return (
    ...
    // Only when it happened: a line that always says "0 unanswered" is a line
    // nobody reads, and this is the one you want to find in the log.
    `${failed > 0 ? `, ${failed} unanswered` : ''})`
  );
}
