...
// `gh pr list` returns an array; `gh pr view` returns an object. A query that
// failed returns null, which is not the same answer as an empty array: an
// offline or timed-out `gh` must not read as "this repo has no open PRs".
function ghList(args, opts) {
  const { ok, data } = ghJson(args, opts);
...
// only for branches that miss here. Null means the query failed.
...
// Three outcomes, and the caller needs all three: a PR, null for "this branch
// has no PR", undefined for "we could not find out".
