...
export const DEFAULTS = Object.freeze({
  pollSeconds: 90,
  // compact, not emoji: herdr draws its own space state as single-width ○/●,
  // and double-width emoji beside them read as oversized. Colour is the
  // trade — set "emoji" if you want it back.
  style: 'compact',
...
// split is a desktop side panel; tab is what works on herdr's single-column
// mobile layout. zoomed and overlay are full-screen variants.
export const CHECKS_PLACEMENTS = ['split', 'tab', 'zoomed', 'overlay'];
...
export function loadConfig(configDir) {
  ...
  return {
    ...
    checksPlacement: CHECKS_PLACEMENTS.includes(raw.checksPlacement)
      ? raw.checksPlacement
      : DEFAULTS.checksPlacement,
    checksRefreshSeconds: num(raw.checksRefreshSeconds, DEFAULTS.checksRefreshSeconds, {
      min: 5,
      max: 600,
    }),
