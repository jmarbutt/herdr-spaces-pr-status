...
// agent state.
export const TOKEN_KEYS = ['pr', 'pr_checks', 'pr_review', 'pr_diff', 'pr_threads'];
...
export function threadsToken(pr, { explicit = false } = {}) {
  if (!pr || TERMINAL_STATES.has(pr.rolled) || !Number.isInteger(pr.resolvableThreads) || pr.resolvableThreads < 0) return null;
  return cap(`${pr.resolvableThreads} ${explicit ? 'resolvable ' : ''}thread${pr.resolvableThreads === 1 ? '' : 's'}`);
}
