...
export function syncOnce({
  ...
  const stats = { spaces: 0, repoQueries: 0, branchQueries: 0, cacheHits: 0, reported: 0, failed: 0 };
  ...
  const current = {};
  // Spaces we could not get an answer for this cycle. They keep their last
  // known PR in the persisted state, so the notifier still has a baseline, but
  // they are deliberately not reported: an unrefreshed token expires on its own
  // TTL, which blanks a space we cannot vouch for instead of leaving a stale
  // status that looks live.
  const unknown = {};

  const carryOver = (space) => {
    stats.failed += 1;
    const previous = (state.spaces ?? {})[space.workspace_id];
    if (previous) unknown[space.workspace_id] = { space, pr: previous.pr ?? null };
  };
  ...
  for (const [repo, repoSpaces] of groupByRepo(spaces)) {
    ...
    stats.repoQueries += 1;
    if (!open) {
      // The repo query failed. Falling through would ask a targeted question
      // per branch on a connection that has just proved it cannot answer one —
      // paying a timeout each time, then caching every "no PR" it invents.
      for (const space of pending) carryOver(space);
      continue;
    }
  ...
  return {
    ok: true,
    error: null,
    state: { spaces: { ...unknown, ...current }, cache, updatedAt: now },
    notifications,
    stats,
  };
}
